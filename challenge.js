...
const keys = [
  ...
  if (isRandomValue(keyPick)) {
    const keyPool = (keys || []).filter(k => !isRandomValue(k));
    keyPick = keyPool.length ? getRandom(keyPool) : "— none found —";
  }
  ...
  cats.forEach(cat => {
    ...
    // A) Attribute selectors (no cat.data)
    if (!cat.data) {
      let pick = val;
