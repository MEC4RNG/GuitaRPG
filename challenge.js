...
//–– Metadata for Level-1 only shown as example; fill out others similarly
const guitarmanshipMeta = {
  ...
  "Hammer-Ons & Pull-Off Foundations": {
...
//–– empty metas you can fill out later
...
//–– 5) Musicianship (Mental Mastery 1–5) ––
const musicianship = {
...
//–– Attributes (both columns) ––
const attributes = [ "Dexterity","Strength","Precision","Intelligence","Creativity","Stamina","Charisma","Wisdom","Flexibility" ];
...
window.addEventListener("DOMContentLoaded", ()=>{
  ...
  btn.onclick     = ()=> generateChallenge(container);
  ...
  const config = {
    ...
    playStyleSelect:     [].concat(...Object.values(playStyle))// challenge.js
...
const guitarmanship = {
  ...
  function generateChallenge(container){
    ...
    // 2) Categories with optional metadata
    const cats = [
  ...
  function appendWithMeta(container, label, choice, metaMap){
    ...
    const ex   = meta.example
      ? `<div class="example"><em>Example:</em> ${meta.example}</div>`
      : "";
    container.innerHTML += `
      <div class="challenge-block">
  ...
  function populateSelectByLevels(selectId, dataObj, categoryKey) {
    const sel    = document.getElementById(selectId);
    ...
    // start fresh
    sel.innerHTML = "";
    // always give them a “Random” first
    sel.append(new Option("Random", "random"));
  
    // pick only the chosen levels, or everything if none
    const pool = levels.length
      ? levels.flatMap(l => dataObj[l] || [])
      : [].concat(...Object.values(dataObj));
  
    pool.forEach(item => sel.append(new Option(item, item)));
  }
  
  // wire it up for every category:
  ["guitarmanship","picking","fretting","string","musicianship","scales","rhythm","playStyle"]
    .forEach(cat => {
      document.querySelectorAll(`input[data-category="${cat}"]`)
              .forEach(cb => cb.addEventListener("change", () =>
                 populateSelectByLevels(cat + "Select",
                                         {                       // choose the right data object
                                           guitarmanship, pickingHand,
                                           frettingHand, stringChallenge,
                                           musicianship, scales, rhythm, playStyle
                                         }[cat],
                                         cat
               )));
      // also do one initial fill on page load:
      populateSelectByLevels(cat + "Select",
                             { guitarmanship, pickingHand,
                               frettingHand, stringChallenge,
                               musicianship, scales, rhythm, playStyle
